...
import type { Athlete, PersonalRecord } from "../types/types";
...
export async function insertPersonalRecord(
	...
	const { error: insertError } = await supabase.from("athlete_prs").insert({
		name: athlete.name,
...
export async function addToPersonalRecordHistory(
	...
	const { error } = await supabase.from("every_pr").insert({
		name: athlete.name,
		id: athlete.id,
		time: pr.time,
		event: pr.event,
...
export async function insertSchoolRecord(
	pr: Omit<PersonalRecord, "delta" | "athlete">,
	name: string,
	year: string,
	outdoor: boolean,
	...
	if (outdoor) {
		await supabase.from("outdoor_school_records").insert({
			event: pr.event,
			time: pr.time,
			year: year,
			name: name,
		...
		await supabase.from("indoor_school_records").insert({
			event: pr.event,
			time: pr.time,
			year: year,
			name: name,
