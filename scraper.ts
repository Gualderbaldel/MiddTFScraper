...
import { HamiltonPage } from "./page_objects/FUCKHAMILTONPage";
...
export async function scrapeMiddTF(page: Page, outdoor: boolean) {
    ...
    if (outdoor) {
        ...
        for (const event of Object.keys(confLeaderboard)) {
            ...
            const newEntries = hamLeaderboards[event].map(performance =>
            ({
                ...performance,
                team: "Hamilton",
                NERank: 0,
            ...
            merged[event] = [...nescacEntries, ...newEntries]
                .sort((a, b) => Number(parseTime(a.time ?? "0")) - Number(parseTime(b.time ?? "0")))
                .map((row, index, rows) => {
                    if (index === 0 || parseTime(rows[index - 1].time ?? "0") !== parseTime(row.time ?? "0")) nescacRank = index + 1
                    return {
