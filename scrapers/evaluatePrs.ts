...
import type { Athlete, Performance, PersonalRecord } from "../types/types";
...
export async function evaluatePrs(
	...
	const { existingAthlete, fetchError } =
		await fetchAllPersonalRecords(athlete);
	if (fetchError) {
		console.error(`Error checking ${athlete.name}:`, fetchError.message);
		return {
			updatedRecords,
	...
	const prMap = new Map(athletePrs.map((pr) => [pr.event, pr]));
	...
	for (const pr of prObjects) {
		...
		if (!existingEvent) {
			...
			if (insertError) {
				console.error(`Error inserting ${athlete.name}:`, insertError.message);
			} else {
				console.log(
					`Inserted ${athlete.name}'s ${pr.event} pr into the database.`,
				);
			}
		} else if (parseTime(existingEvent.time) > parseTime(pr.time)) {
			await updatePersonalRecord(pr, athlete);
			const delta = (
				parseTime(pr.time) - parseTime(existingEvent.time)
			).toFixed(2);
			console.log(
				`Athlete: ${athlete.name} PR of ${pr.time} for ${pr.event} updated (delta of ${delta}s)`,
			);
			updatedRecords.push({
				time: pr.time,
				event: pr.event,
			...
			const { error } = await addToPersonalRecordHistory(
				pr,
				athlete,
				Number(delta),
				existingEvent.time,
			...
			const { schoolRecord, fetchError } = await fetchSchoolRecord(
				pr.event,
				outdoor,
			);
			if (fetchError) {
				console.error(
					"oopsies there is this error with grabbing the school record:",
					fetchError.message,
				);
			}
			...
			if (parseTime(pr.time) <= parseTime(schoolRecord.time)) {
				...
				console.log(
					`New School Record for ${athlete.name} in the ${pr.event} with a time of ${pr.time}`,
				);
				newSchoolRecords.push({
					time: pr.time,
					event: pr.event,
					yearsSinceBroken: String(
						Number(thisYear) - Number(schoolRecord.year),
					),
					athlete: {
						name: athlete.name,
						id: athlete.id,
				...
				console.log(`not a school Record for ${athlete.name} in ${pr.event}`);
		}
	}
	return {
		updatedRecords,
