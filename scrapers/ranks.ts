...
import type { Leaderboard, LeaderboardEntry } from "../types/types";

export async function athleteRankings(
	ranks: Leaderboard,
	outdoor: boolean,
): Promise<Leaderboard> {
	...
	for (const [event, rows] of Object.entries(ranks)) {
		...
		const { data, error: fetchError } = await supabase
			...
			.eq("event", event);
		if (fetchError) {
			console.error(`Error checking rows:`, fetchError.message);
			continue;
		}
		let eventData = data;
		const returnedAthletes = new Set(eventData.map((athlete) => athlete.name));
		const missingAthletes = rows.filter(
			(athlete) => !returnedAthletes.has(athlete.athlete),
		);
		if (missingAthletes.length > 0) {
			console.log(
				`These athletes are not in the rankings database for the ${event}: `,
				missingAthletes.map((athlete) => athlete.athlete),
			);
			...
			if (error) {
				...
				console.log(
					"athletes added to rankings: ",
					missingAthletes.map((athlete) => athlete.athlete),
				);
			eventData = [...eventData, ...(insertedData ?? [])];
		}
		const athletesByName = new Map(
			rows.map((athlete) => [athlete.athlete, athlete]),
		);
		...
		for (const row of eventData) {
			...
			if ((athlete.nationalRank ?? null) !== row.national_rank) {
				...
				console.log(
					`updating national rank for ${athlete.athlete} from ${row.national_rank} to ${athlete.nationalRank}`,
				);
			}
			...
			if ((athlete.NERank ?? null) !== row.ne_rank) {
				...
				console.log(
					`updating nescac rank for ${athlete.athlete} from ${row.ne_rank} to ${athlete.NERank}`,
				);
			}
			...
			if ((athlete.nescacRank ?? null) !== row.nescac_rank) {
				...
				console.log(
					`updating nescac rank for ${athlete.athlete} from ${row.nescac_rank} to ${athlete.nescacRank}`,
				);
			}
			...
			if (Object.keys(updates).length > 0) {
				...
				const existing = rows.find((row) => row.athlete === athlete.athlete);
